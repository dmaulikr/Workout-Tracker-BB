...
- (void)addSession1ToExistingCoreDataObjects {
    ...
    // WorkoutCompleteDate Entity
    context = [[CoreDataHelper sharedHelper] context];
    ...
    for (WorkoutCompleteDate *info in objects) {
    ...
    if (objectsUpdatedCount != 0) {
        ...
        if (debug==1) {
            NSLog(@"WorkoutCompleteDate ObjectsUpdatedCount = %d", objectsUpdatedCount);
        }
