...
class WorkoutTVC: UITableViewController, UIPopoverPresentationControllerDelegate, UIPopoverControllerDelegate, MFMailComposeViewControllerDelegate, MPAdViewDelegate {
    ...
    fileprivate struct CellType {
        static let straight = "WorkoutCell_Straight"
        static let triangle = "WorkoutCell_Triangle"
        static let dropSet = "WorkoutCell_DropSet"
    ...
    func wasGraphViewPurchased() -> Bool {
        ...
        if Products.store.isProductPurchased("com.grantsoftware.90DWTBB.graphview") {
        ...
        if let workoutObject = cellArray[indexPath.section][indexPath.row] as? NSArray {
            ...
            if let cellIdentifierArray = workoutObject[5] as? [String] {
                ...
                if cellIdentifier != "CompletionCell" {
                    ...
                    if let workoutObjects = CDOperation.getWeightTextForExercise(localSession, routine: workoutRoutine, workout: selectedWorkout, exercise: titleArray![0] as! String, index: localIndex as NSNumber)  as? [Workout] {
